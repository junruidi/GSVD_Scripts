...
pct_ev3_v = cumsum(svd_3_v)/sum(svd_3_v) # 9

core3_u = hosvd3_u$middle
svd_3_u = svd(core3_u)$d
pct_ev3_u = cumsum(svd_3_u)/sum(svd_3_u) # 10
...
pct_ev4_v = cumsum(svd_4_v)/sum(svd_4_v) # 10

core4_u = hosvd4_u$middle
svd_4_u = svd(core4_u)$d
pct_ev4_u = cumsum(svd_4_u)/sum(svd_4_u) # 11
...
# plots 
pdf(file = "Write Up/cor_gsvd_nocenter.pdf", width = 28, height = 28)
par(mar = c(4,5,9,6))
par(oma = c(1,0,1,0))
corrplot::corrplot(cor(U),cl.pos = "b",cl.cex = 2,tl.cex = 1.6,cl.align.text = "r",na.label = "-")
dev.off()

pdf(file = "Write Up/S_nocenter.pdf", width = 15, height = 15)
par(mfrow = c(3,3))
...
pdf("Write Up/gsvd_V_nocenter.pdf",width = 10,height = 10)
par(mfrow = c(3,1))
for(i in 1:32){
...
survival_gsvd_nocenter = cbind(cov50,U)

library(survey)
svydata_gsvdnoc = svydesign(id=~SDMVPSU,
                          strat=~SDMVSTRA,
                          weight=~wt4yr_norm,
                          nest=TRUE,
                          data=survival_gsvd_nocenter)

save(svydata_gsvdnoc, file = "Data/survival_gsvdnoc.dta")
