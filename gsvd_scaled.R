...
pct_ev3_u = cumsum(svd_3_u)/sum(svd_3_u) 
...
V = as.data.frame(cbind(V3,V4))
names(V) = paste0("V",rep(c(3,4),each = 32),"_",rep(c(1:32),2))

U = as.data.frame(cbind(u,U3,U4))
names(U) = paste0("U",rep(c(2,3,4),each = 32),"_",rep(c(1:32),2))
...
pdf(file = "Write Up/S_scaled.pdf", width = 15, height = 15)
par(mfrow = c(3,3))
...
pdf("Write Up/gsvd_V_scaled_center.pdf",width = 10,height = 10)
par(mfrow = c(3,1))
for(i in 1:32){
  plot(v[,i],main = paste0("V2 - ",i),type = "l",xaxt = "n",ylab = "RSV")
